...
from utils import wasserstein_loss, visualize_graph_layout, plot_graph_layout, contrastive_layout_loss, perceptual_layout_loss
...
def compute_gradient_penalty(discriminator, real_samples, fake_samples, real_adj, fake_adj, device, lambda_gp=5):
    """
    ...
    d_interpolates, _ = discriminator(interpolated_samples, interpolated_adj)

    # Compute gradients of the discriminator's output with respect to the interpolated samples
    # Use allow_unused=True to handle cases where some inputs might not be used in the computation graph
    ...
    # Handle the case where gradients might be None for unused inputs
    gradients_list = []
    ...
    if not gradients_list:
        # If no gradients were computed, return zero penalty
        return torch.tensor(0.0, device=device, requires_grad=True)
    
    # Concatenate all valid gradients
    if len(gradients_list) == 1:
        gradients = gradients_list[0]
    else:
        gradients = torch.cat([g.view(batch_size, -1) for g in gradients_list], dim=1)
...
def train_gan(generator, discriminator, data_loader, optimizer_g, optimizer_d, device, epochs, output_dir, 
              lambda_gp=5, lambda_rec=100, patience=20):
    ...
    with open(log_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['epoch', 'generator_loss', 'reconstruction_loss','discriminator_loss','discriminator_accuracy','adv_loss','contrastive_loss','perceptual_loss'])
    ...
    for epoch in range(epochs):
        ...
        # Initialize monitoring variables for new loss components
        total_adv_loss = 0
        ...
        for batch_idx, (node_coords, adj_matrices,  _) in enumerate(data_loader):
            ...
            num_nodes_tensor = non_zero_nodes_mask.sum(dim=1)  # Shape: (96,) - number of nodes for each graph in the batch
            num_nodes_list = num_nodes_tensor.tolist()  # Convert to list for indexing

            #generated_coords = generator(z, node_coords, num_nodes)
            generated_coords = generator(z, node_coords, adj_matrices, num_nodes_list)
            #real_validity = discriminator(node_coords)
            real_validity, real_embeddings = discriminator(node_coords, adj_matrices)
            #fake_validity = discriminator(generated_coords.detach())
            fake_validity, fake_embeddings = discriminator(generated_coords.detach(), adj_matrices)  # Detach to avoid updating generator
            ...
            if not stop_training_d:
                # Discriminator loss (WGAN-GP adversarial loss)
                discriminator_loss = wasserstein_loss(real_validity, fake_validity)
                
                # Contrastive Layout Loss (InfoNCE style)
                contrastive_loss = contrastive_layout_loss(real_embeddings, fake_embeddings, temperature=0.1)
                
                # Perceptual Layout Loss
                perceptual_loss = perceptual_layout_loss(real_embeddings, fake_embeddings)
                
                # Compute gradient penalty
                gp = compute_gradient_penalty(discriminator, node_coords, generated_coords, adj_matrices, adj_matrices, device, lambda_gp)
                
                # Total discriminator loss: L_D = L_adv + λ_contr * L_contr + λ_perc * L_perc
                lambda_contr = 0  # Weight for contrastive loss
                lambda_perc = 0   # Weight for perceptual loss
                d_loss = discriminator_loss + lambda_contr * contrastive_loss + lambda_perc * perceptual_loss + gp
                
                # Store individual loss components for monitoring
                total_adv_loss += discriminator_loss.item()
            ...
            if not stop_training_g:
                ...
                fake_validity, fake_embeddings_gen = discriminator(generated_coords, adj_matrices)
            ...
            if batch_idx % 30 == 0:  # Visualize and save every 10 batches
                for i, layout in enumerate(generated_coords[:3]):  # Visualize the first 3 generated layouts
                    ...
                    img = plot_graph_layout(
                        layout[:num_nodes_list[i]].detach().cpu().numpy(),
        ...
        # Calculate average loss components for monitoring
        avg_adv_loss = total_adv_loss / len(data_loader)
        avg_contrastive_loss = total_contrastive_loss / len(data_loader)
        avg_perceptual_loss = total_perceptual_loss / len(data_loader)
        ...
        # Detailed loss breakdown for hyperparameter tuning
        print(f"\n--- Detailed Loss Breakdown (Epoch {epoch + 1}) ---")
        print(f"Adversarial Loss: {avg_adv_loss:.4f}")
        print(f"Contrastive Loss: {avg_contrastive_loss:.4f} (λ_contr = {lambda_contr})")
        print(f"Perceptual Loss: {avg_perceptual_loss:.4f} (λ_perc = {lambda_perc})")
        print(f"Gradient Penalty: {gp.item():.4f}")
        print(f"Total D Loss: {avg_d_loss:.4f}")
        # Calculate loss ratios safely to avoid division by zero
        if abs(avg_adv_loss) < 1e-8:
            print("Loss Ratios - Adv:Contr:Percep = 1:0:0 (Adversarial loss is negligible)")
            print("Training Status: Generator and discriminator in perfect balance")
        else:
            adv_contr_ratio = lambda_contr * avg_contrastive_loss / avg_adv_loss
            adv_perc_ratio = lambda_perc * avg_perceptual_loss / avg_adv_loss
            print(f"Loss Ratios - Adv:Contr:Percep = 1:{adv_contr_ratio:.2f}:{adv_perc_ratio:.2f}")
        print("-" * 50)
        ...
        with open(log_path, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([epoch + 1, avg_g_loss,avg_reconstruction_loss, avg_d_loss, discriminator_accuracy, avg_adv_loss, avg_contrastive_loss, avg_perceptual_loss])
...
def train_combined_cgan(generator, discriminator, classifier, train_loader, val_loader, test_loader, 
                        ...
                        device, epochs=100, patience=15, output_dir=".", lambda_gp=5, alpha=30, num_z_samples=10):
    os.makedirs(output_dir, exist_ok=True)
    ...
    with open(log_path, 'w', newline='') as f:
        ...
        writer.writerow(["epoch", "generator_loss", "discriminator_loss", "classifier_loss", "train_accuracy",
                        "train_f1", "val_accuracy", "val_f1", "test_accuracy", "test_f1", "adv_loss", "contrastive_loss", "perceptual_loss"])
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (graph_layouts, adj_matrices, labels) in enumerate(train_loader):
            ...
            #             # Train Discriminator
            real_validity, real_embeddings = discriminator(graph_layouts, adj_matrices)
            ...
            for _ in range(num_z_samples):
                ...
                generated_layouts = generator(z, graph_layouts, adj_matrices, num_nodes=num_nodes_list)
                # Detach so G won't be updated in this pass
                fv, fe = discriminator(generated_layouts.detach(), adj_matrices)
                fake_validities.append(fv)
                fake_embeddings_list.append(fe)
            
            # Average across z-samples
            fake_validity = torch.stack(fake_validities, dim=0).mean(dim=0)
            fake_embeddings = torch.stack(fake_embeddings_list, dim=0).mean(dim=0)
            ...
            for _ in range(num_z_samples):
                z = torch.randn(graph_layouts.shape[0], 128).to(device)
                gen_layouts = generator(z, graph_layouts, adj_matrices, num_nodes=num_nodes_list)

                # Adversarial loss for generator
                f_validity, f_embeddings = discriminator(gen_layouts, adj_matrices)
                ...
                for i, layout in enumerate(gen_layouts):
                    ...
                    img = visualize_graph_layout(layout[:num_nodes_list[i]], adj_matrix=adj_matrix, size=(224, 224))  # Get image tensor directly
                    images.append(img)
            ...
            # # Add contrastive and perceptual losses to generator for better layout quality
            # # Note: We use the embeddings from the current generator forward pass
            # g_contrastive_loss = contrastive_layout_loss(real_embeddings, fake_embeddings, temperature=0.1)
            # g_perceptual_loss = perceptual_layout_loss(real_embeddings, fake_embeddings)
            
            # # Combine all generator losses
            # g_loss = g_loss + lambda_contr * g_contrastive_loss + lambda_perc * g_perceptual_loss
        ...
        with open(log_path, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([epoch + 1, avg_g_loss, avg_d_loss, avg_c_loss, train_accuracy, avg_f1_train, val_accuracy, val_f1, test_accuracy, test_f1, avg_adv_loss, avg_contrastive_loss, avg_perceptual_loss])
...
def evaluate_model(generator, classifier, test_loader, device, num_z_samples=10):
    generator.eval()
    classifier.eval()
    correct = 0
    total = 0
